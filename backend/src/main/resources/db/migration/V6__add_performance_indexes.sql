...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tokens_patient_active_status
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tokens_doctor_waiting
    ON tokens (doctor_id, status)
    WHERE status = 'WAITING';

-- No-show expiry job: findByStatusAndCalledAtBefore(CALLED, cutoff)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tokens_called_expiry
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tokens_completed_history
